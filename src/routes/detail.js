var errors = require('../lib/errors'),
    ...
    databaseLookup = require('../lib/db/lookup'),
...
exports.bridge = function (collections) {
    return function (req, res) {
        var fingerprint = req.params.fingerprint,
            lookupFunction;

        if (connection.isLocked()) {
            logger.info('bridge', 'using onionoo lookup');
            lookupFunction = lookup(fingerprint, false);
        } else {
            logger.info('bridge', 'using database lookup');
            lookupFunction = databaseLookup(collections, fingerprint, false);
        }

        lookupFunction.then(function (detail) {
            var bridge,
...
exports.relay = function (relayCollections) {
    return function (req, res) {
        var fingerprint = req.params.fingerprint,
            lookupFunction;
        ...
        if (connection.isLocked()) {
            // use onionoo
            logger.info('relay', 'using onionoo lookup');
            lookupFunction = lookup(fingerprint, false);
        } else {
            // use database
            logger.info('relay', 'using database lookup');
            lookupFunction = databaseLookup(relayCollections, fingerprint, false);
        }
